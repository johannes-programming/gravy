...
test: beautiful
	conda run -n base python make/env.py test_gravy --python=3.11 --recreate >/dev/null;
	conda run -n test_gravy pip install -e . >/dev/null;
	conda run -n test_gravy python -c 'import gravy.tests; gravy.tests.test();'
	conda run -n test_gravy pip install mypy >/dev/null;
	conda run -n test_gravy python -m mypy --exclude build --strict .;
	conda run -n test_gravy python -m mypy --strict -p gravy;
